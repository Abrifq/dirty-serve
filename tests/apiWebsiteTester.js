const { add: addAPI, findFirstMatchingAPI, remove: removeAPI } = require("../apiHandlerPool").interface,
    { asyncHTTPRequest } = require("./common"), mimetype = require("mimetype");
...
const jsonMimeType = mimetype.lookup(".json");
const htmlMimeType = mimetype.lookup(".html");
const plainTextMimeType = mimetype.lookup(".txt");
/**@type {import("../apiHandlerPool").APICallback} */
const blankAPI = (req, res) => {
    const acceptType = "accept" in req.headers ?
        ...
        undefined;
    res.statusCode = 200;
    res.statusMessage = undefined; //default message is generated by status code 
    switch (acceptType) {
        case htmlMimeType:
...
exports.test = async function (port) {
...

    {
        ...
        const apiPath = "/api/null";
        const apiID = addAPI(apiPath, blankAPI);
        try {
